...
from app.agent.nodes.extract_query_slots import extract_query_slots
...
graph_builder.add_node("extract_query_slots", extract_query_slots)
graph_builder.add_node("add_extra_context", add_extra_context)
graph_builder.add_node("generate_sql", generate_sql)
graph_builder.add_node("validate_sql", validate_sql)
graph_builder.add_node("validate_sql_slot", validate_sql_slot)
graph_builder.add_node("correct_sql", correct_sql)
graph_builder.add_node("run_sql", run_sql)
graph_builder.add_node("final_error", final_error)
...
# 表和指标都过滤完成后，并行补充额外上下文并抽取用户问题语义槽位
# 两者都依赖过滤后的 table_infos/metric_infos，且彼此独立，可并行执行
graph_builder.add_edge("filter_table", "add_extra_context")
graph_builder.add_edge("filter_metric", "add_extra_context")
graph_builder.add_edge("filter_table", "extract_query_slots")
graph_builder.add_edge("filter_metric", "extract_query_slots")

# 额外上下文和语义槽位都就绪后再生成 SQL
graph_builder.add_edge("add_extra_context", "generate_sql")
graph_builder.add_edge("extract_query_slots", "generate_sql")
graph_builder.add_edge("generate_sql", "validate_sql")

# 第一道：语法校验。通过进入语义槽位校验，失败且未超限则进入修正，超限进入最终错误
graph_builder.add_conditional_edges(
    source="validate_sql",
    path=route_after_validate_sql,
    path_map={
        "validate_sql_slot": "validate_sql_slot",
        "correct_sql": "correct_sql",
        "final_error": "final_error",
    },
)

# 第二道：语义槽位校验。一致就执行；不一致且未超限则修正，超限进入最终错误
graph_builder.add_conditional_edges(
    source="validate_sql_slot",
    path=route_after_validate_sql_slot,
    path_map={
        "correct_sql": "correct_sql",
        "run_sql": "run_sql",
        "final_error": "final_error",
...
# 修正后回到语法校验重新走完整校验链路，形成带上限的校验-修正闭环
graph_builder.add_edge("correct_sql", "validate_sql")
graph_builder.add_edge("run_sql", END)
graph_builder.add_edge("final_error", END)
